...
static inline Vec2 leftOf(Rect body, Rect frame) {
    return Vec2{frame.origin.x - body.size.width, frame.origin.y};
}

static inline Vec2 rightOf(Rect body, Rect frame) {
    return Vec2{frame.origin.x + frame.size.width, frame.origin.y};
}

static inline Vec2 above(Rect body, Rect frame) {
    return Vec2{frame.origin.x, frame.origin.y + frame.size.height};
}

static inline Vec2 below(Rect body, Rect frame) {
    return Vec2{frame.origin.x, frame.origin.y - body.size.height};
}
